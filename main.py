...
cosine_sim_item = pd.read_parquet("Archivos_para_SISTEMA_DE_RECOMENDACIÓN/cosine_sim_item.parquet")
...
@app.get("/cosine_sim_item/{game}")
def recomendacion_juego(game):
    # Filtrar los datos del DataFrame por el juego proporcionado
    game_filtered = cosine_sim_item[cosine_sim_item['item_name'] == game]

    # Obtener la lista de juegos similares ordenados
    similar_games = cosine_sim_item.sort_values(by=game, ascending=False).iloc[1:6]

    count = 1
    recomendaciones = {}
    
    for index, row in similar_games.iterrows():
        item_name = row['item_name']  # Ajusta el nombre de la columna según tu DataFrame
        recomendaciones[count] = item_name
        count += 1
